...
//#pragma omp parallel for num_threads(outer_threads) schedule(dynamic)
	for(int realis = 0; realis < this->realisations; realis++)
	{
		...
		// ------------------------------------- calculate level statistics
			...
			// std::cout << eigenvalues_unfolded.t();
		// ------------------------------------- calculate sff
			...
			if(this->fun == 1) std::cout << "\t\t	--> finished raw SFF for " << prefix + info << " - in time : " << tim_s(start) << "s" << std::endl;
			start = std::chrono::system_clock::now();
		...
		// #pragma omp parallel for
			...
			if(this->fun == 1) std::cout << "\t\t	--> finished filtered SFF for " << prefix + info << " - in time : " << tim_s(start) << "s" << std::endl;
			start = std::chrono::system_clock::now();
		...
		// 	sff_r_folded.save(arma::hdf5_name(dir_re + info + ".hdf5", "sff_fold_eta=0.3", arma::hdf5_opts::append));
		// 	arma::vec({Z_r_folded}).save(arma::hdf5_name(dir_re + info + ".hdf5", "Z_fold_eta=0.3", arma::hdf5_opts::append));
		// 	sff_r_folded2.save(arma::hdf5_name(dir_re + info + ".hdf5", "sff_fold_eta=0.5", arma::hdf5_opts::append));
		// 	arma::vec({Z_r_folded2}).save(arma::hdf5_name(dir_re + info + ".hdf5", "Z_fold_eta=0.5", arma::hdf5_opts::append));
	...
	for(int b_idx = 0; b_idx < betas.size(); b_idx++){
		sff_beta.row(b_idx) = sff_beta.row(b_idx) / Z_beta(b_idx);
		sff_beta_raw.row(b_idx) = sff_beta_raw.row(b_idx) / Z_beta_raw(b_idx);
		sff_beta_folded.row(b_idx) = sff_beta_folded.row(b_idx) / Z_beta_folded(b_idx);
	}
	...
	betas.save(arma::hdf5_name(dir + info + ".hdf5", "betas", arma::hdf5_opts::append));
	sff_beta.save(arma::hdf5_name(dir + info + ".hdf5", "sff_beta", arma::hdf5_opts::append));
	sff_beta_raw.save(arma::hdf5_name(dir + info + ".hdf5", "sff_beta_raw", arma::hdf5_opts::append));
	sff_beta_folded.save(arma::hdf5_name(dir + info + ".hdf5", "sff_beta_folded", arma::hdf5_opts::append));
	...
	sff_eta_folded.save(arma::hdf5_name(dir + info + ".hdf5", "sff_eta_folded", arma::hdf5_opts::append));

	sff_raw.save(arma::hdf5_name(dir + info + ".hdf5", "sff_raw", arma::hdf5_opts::append));
	sff_raw_folded.save(arma::hdf5_name(dir + info + ".hdf5", "sff_raw_folded", arma::hdf5_opts::append));
	...
	arma::uvec({counter}).save(arma::hdf5_name(dir + info + ".hdf5", "realisations", arma::hdf5_opts::append));
	// #endif
...
// #pragma omp parallel for num_threads(outer_threads) schedule(dynamic)
	for(int realis = 0; realis < this->realisations; realis++)
	{
		...
		for(int i = 0; i < sites.size(); i++)
		{
			...
			if(site < this->L - 1){
				...
				typ_susc_SxSx_r(i) = _typ_susc;
				diag_mat_elem_SxSx_r.col(i) = arma::diagvec(mat_elem); 
		...
		// #ifndef MY_MAC
		{
			...
			agp_norm_SxSx_r.save( arma::hdf5_name(dir_realis + info + ".hdf5", "AGP/SxSx", arma::hdf5_opts::append));
			...
			typ_susc_SxSx_r.save( arma::hdf5_name(dir_realis + info + ".hdf5", "TYP_SUSC/SxSx", arma::hdf5_opts::append));
			...
			diag_mat_elem_SxSx_r.save( arma::hdf5_name(dir_realis + info + ".hdf5", "DIAG_MAT/SxSx", arma::hdf5_opts::append));
	...
	if(counter == 0) return;
		...
		agp_norm_SxSx.save( arma::hdf5_name(dir + info + ".hdf5", "AGP/SxSx", arma::hdf5_opts::append));
		...
		typ_susc_Sx.save(	arma::hdf5_name(dir + info + ".hdf5", "TYP_SUSC/Sx",   arma::hdf5_opts::append));
		typ_susc_SzSz.save( arma::hdf5_name(dir + info + ".hdf5", "TYP_SUSC/SzSz", arma::hdf5_opts::append));
		typ_susc_SxSx.save( arma::hdf5_name(dir + info + ".hdf5", "TYP_SUSC/SxSx", arma::hdf5_opts::append));
		...
		diag_mat_elem_Sx.save(   arma::hdf5_name(dir + info + ".hdf5", "DIAG_MAT/Sx",   arma::hdf5_opts::append));
		diag_mat_elem_SzSz.save( arma::hdf5_name(dir + info + ".hdf5", "DIAG_MAT/SzSz", arma::hdf5_opts::append));
		diag_mat_elem_SxSx.save( arma::hdf5_name(dir + info + ".hdf5", "DIAG_MAT/SxSx", arma::hdf5_opts::append));
