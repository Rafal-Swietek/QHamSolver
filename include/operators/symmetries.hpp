...
namespace QOps{
    ...
    inline
    generic_operator<cpx>
    symmetry(unsigned int L, __builtin_operators gen, cpx sym_eig_val, int arg = 1)
    { 
        auto _kernel = choose_symmetry<_particle_type>(gen, L, arg); 
        return generic_operator<cpx>(L, _kernel, sym_eig_val);
    }
    ...
    { 
        ...
        // if(inverse) return symmetry<_particle_type>(L, __builtin_operators::Tinv, std::cos(ksym) + im * std::abs(sin(ksym)), shift);
        // else        return symmetry<_particle_type>(L, __builtin_operators::T,    std::cos(ksym) - im * std::abs(sin(ksym)), shift);
        if(inverse) return symmetry<_particle_type>(L, __builtin_operators::Tinv, std::exp(im * ksym), shift);
        else        return symmetry<_particle_type>(L, __builtin_operators::T,    std::exp(-im * ksym), shift);
    }
    ...
    auto _parity_symmetry(unsigned int L, int sector)
        { _assert_((sector == -1 || sector == 1), NOT_ALLOWED_SYM_SECTOR);
          return symmetry<_particle_type>(L, __builtin_operators::P, (double)sector); }
    ...
    auto _spin_flip_x_symmetry(unsigned int L, int sector)
        { _assert_((sector == -1 || sector == 1), NOT_ALLOWED_SYM_SECTOR);
          return symmetry<_particle_type>(L, __builtin_operators::Zx, (double)sector); }
    ...
    auto _spin_flip_y_symmetry(unsigned int L, int sector)
        { _assert_((sector == -1 || sector == 1), NOT_ALLOWED_SYM_SECTOR);
          return symmetry<_particle_type>(L, __builtin_operators::Zy, (double)sector); }
    ...
    auto _spin_flip_z_symmetry(unsigned int L, int sector)
        { _assert_((sector == -1 || sector == 1), NOT_ALLOWED_SYM_SECTOR);
          return symmetry<_particle_type>(L, __builtin_operators::Zz, (double)sector); }
