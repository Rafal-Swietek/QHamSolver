...
def info_dis(L, J1=0, J2=0, delta1=0, delta2=0, hz=0, w=0, pb=0):
    """
    Main body of file names with model parameters as input for disordered
    """
    info = info_base(L, J1, J2, delta1, delta2, hz)
    n = order_of_magnitude(w)
    info +=  str(",w={:.%df}"%n).format(round(w, n))
    info += ",pb=%d"%pb
    return info

def info_base(L, J1=0, J2=0, delta1=0, delta2=0, hz=0):
    """
...
def info_sym(L, J1=0, J2=0, delta1=0, delta2=0, hz=0, k=0, p=1, zx=1, Sz=0.0, BOUNDARY_COND = 'PBC'):
    """
