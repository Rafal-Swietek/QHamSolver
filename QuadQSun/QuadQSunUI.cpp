...
typename ui::model_pointer ui::create_new_model_pointer(){
    return std::make_unique<QHS::QHamSolver<QuadQSun>>(this->L_loc, this->J, this->alfa, this->gamma, this->w, this->h, 
...
void ui::reset_model_pointer(){
    this->ptr_to_model.reset(new QHS::QHamSolver<QuadQSun>(this->L_loc, this->J, this->alfa, this->gamma, this->w, this->h, 
																	this->seed, this->grain_size, this->zeta, this->initiate_avalanche, normalize_grain)); 
}
...
void ui::parse_cmd_options(int argc, std::vector<std::string> argv)
{
    ...
    user_interface_dis<QuadQSun>::parse_cmd_options(argc, argv);
...
/// @brief 
void ui::set_default(){
    user_interface_dis<QuadQSun>::set_default();
    this->J = 1.0;
	this->Js = 0.0;
	this->Jn = 1;
...
void ui::print_help() const {
    user_interface_dis<QuadQSun>::print_help();
...
void ui::printAllOptions() const{
    user_interface_dis<QuadQSun>::printAllOptions();
	std::cout << "QUADRATIC QUANTUM SUN:\n\t\t" << "H = H = \u03A3_{ij \in grain} A_{ij} c_i^+ c_j + J\u03A3_i \u03B1^{u_i} [ c_i^+ c_{ni} + h.c. ] + \u03A3_i h_i  c_i^+ c_i";
	std::cout << "\u03A3_i h_i S^z_i" << std::endl << std::endl;
	std::cout << "u_i \u03B5 [j - \u03B6, j + \u03B6]"  << std::endl;
	std::cout << "h_i \u03B5 [h - w, h + w]" << std::endl;
	

	std::cout << "------------------------------ CHOSEN QuadQSun OPTIONS:" << std::endl;
    std::cout 
